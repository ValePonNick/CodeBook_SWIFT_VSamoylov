...
printStudentInfo(students: studentArray)
// Имя: Ann, Возраст: 21, Специальность: IT
...
1. typealias Student = (name: String, age: Int, speciality: String) - эта строка создает псевдоним типа 'Student' для кортежа, содержащего имя, возраст и спец-ть студента.
    ...
    - избегай ОШИБОК + упрощай поддержку кода:
    - если нужно ИЗМЕНИТЬ структуру кортежа(напр. добавить поле "курс") -> нужно изменить ТОЛЬКО определение псевдонима 'Student', а НЕ ВСЕ места, где используется этот кортеж.
    - СЕМАНТИЧЕСКАЯ ясность: псевдоним 'Student' сразу дает понять: кортеж содержит инфу о студенте.
2. функция  'printStudentsInfo(students: Student)' принимает МАССИВ КОРТЕЖЕЙ типа 'Student'.
     - Она ПЕРЕБИРАЕТ масссив и для КАЖДОГО студента
     - вызывает функцию 'printStudentInfo' чтобы вывести его данные в консоль.
